...
//                 <div className="js_container row pt-4">
...
const ManagerList = () => {
  ...
  // Fetch managers from backend when component mounts
  useEffect(() => {
    MyAxios.get('/users')
      .then(response => {
      ...
      .catch(error => {
        ...
        setError('There was an error fetching the managers!');
      });
  }, []);
  ...
  const handleAddManager = () => {
    ...
    if (editIndex !== null) {
      MyAxios.put(`/users/${managers[editIndex].id}`, newManager)
        .then(response => {
          const updatedManagers = [...managers];
          updatedManagers[editIndex] = response.data;
          setManagers(updatedManagers);
          handleCloseAddModal();
        })
        .catch(error => {
          console.error('There was an error updating the manager!', error);
          setError('There was an error updating the manager!');
        });
    } else {
      MyAxios.post('/users', newManager)
        .then(response => {
        ...
        .catch(error => {
          ...
          setError('There was an error adding the manager!');
        });
    }
  };
  ...
  const handleDeleteManager = (index) => {
    ...
    MyAxios.delete(`/users/${managerId}`)
      .then(response => {
        const updatedManagers = [...managers];
        updatedManagers.splice(index, 1);
        setManagers(updatedManagers);
      ...
      .catch(error => {
        ...
        setError('There was an error deleting the manager!');
      });
  };
